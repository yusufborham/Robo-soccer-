...
void controlHmotors() {
    for (int num = 0; num <= 3; num++) {
      if (outputHorizontalThrusters[num] >= 0) {
        analogWrite(HorizontalThrusterSp1[num], int(abs(outputVerticalThrusters[num])));  // control speed
        analogWrite(HorizontalThrusterSp2[num], 0);                                       // control speed
      } else {
        analogWrite(HorizontalThrusterSp1[num], 0);                                       // control speed
        analogWrite(HorizontalThrusterSp2[num], int(abs(outputVerticalThrusters[num])));  // control speed
      }
    }
  }
